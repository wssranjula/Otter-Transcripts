...
class RAGQueryHelper:
    ...
    def get_recent_important_chunks(self, limit: int = 5) -> List[Dict]:
        """
        Get recent high-importance chunks (for temporal queries)

        Args:
            limit: Max number of chunks

        Returns:
            List of recent important chunks
        """
        with self.driver.session() as session:
            result = session.run("""
                MATCH (c:Chunk)
                ...
                RETURN c.id as chunk_id,
                       ...
                       c.chunk_type as type,
                       c.importance_score as importance
                ORDER BY c.meeting_date DESC, c.importance_score DESC
                LIMIT $limit
            """, limit=limit)

            return [dict(record) for record in result]

    def get_chunks_by_date(self, date: str, limit: int = 10) -> List[Dict]:
        """
        Get chunks from a specific date

        Args:
            date: ISO date string (e.g., "2025-06-11")
            limit: Max number of chunks

        Returns:
            List of chunks from that date
        """
        with self.driver.session() as session:
            result = session.run("""
                MATCH (c:Chunk)
                WHERE c.meeting_date = $date
                RETURN c.id as chunk_id,
                       c.text as text,
                       c.meeting_title as meeting,
                       c.meeting_date as date,
                       ...
                       c.chunk_type as type,
                       c.importance_score as importance
                ORDER BY c.importance_score DESC, c.sequence_number ASC
                LIMIT $limit
    ...
    def build_rag_context(self, query: str, entity_names: List[str] = None, limit: int = 5) -> str:
        ...
        temporal_keywords = ['last meeting', 'recent', 'latest', 'most recent', 'last discussed']
        is_temporal = any(keyword in query_lower for keyword in temporal_keywords)

        # Check for date patterns (June 11, May 28, etc.)
        import re
        date_patterns = [
            (r'june\s*11', '2025-06-11'),
            (r'jun\s*11', '2025-06-11'),
            (r'may\s*28', '2025-05-28'),
            (r'6/11', '2025-06-11'),
            (r'5/28', '2025-05-28')
        ]

        target_date = None
        for pattern, iso_date in date_patterns:
            if re.search(pattern, query_lower):
                target_date = iso_date
                break

        # If specific date mentioned, get chunks from that date
        if target_date:
            chunks = self.get_chunks_by_date(target_date, limit=limit)
        else:
            # Search chunks by full-text
            chunks = self.search_chunks_full_text(query, limit=limit)

        # If temporal query and not enough results, also get recent high-importance chunks
        if is_temporal and len(chunks) < limit:
            recent_chunks = self.get_recent_important_chunks(limit=limit)
    ...
    def build_rag_context_with_metadata(self, query: str, entity_names: List[str] = None, limit: int = 5) -> List[Dict]:
        """
        Build context for AI prompt AND return full chunk metadata for UI display
        ...
        Returns:
            List of chunk dictionaries with full metadata including entities
        """
        ...
        # Enrich with entities mentioned in each chunk
        with self.driver.session() as session:
